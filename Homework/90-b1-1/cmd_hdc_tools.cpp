...
/***************************************************************************
  函数名称：hdc_draw_image
  功    能：在指定位置绘制图片
  输入参数：int x, int y - 图片左上角坐标
		   const char* image_path - 图片文件路径
  返 回 值：无
  说    明：使用stb_image库读取图片，需包含stb_image.h
***************************************************************************/
void hdc_draw_image(int x, int y, int a, const char* image_path) {
	...
	if (channels < 3) {
		...
		MessageBoxA(NULL, "Image must have RGB channels", "Error", MB_ICONERROR);
		return;
	}
	...
	for (int j = 0; j < height; j+=a) {
		for (int i = 0; i < width; i+=a) {
			...
			// 如果图片是RGBA，处理Alpha通道（这里简单忽略透明）
			if (channels == 4 && data[idx + 3] == 0) {
				continue; // 跳过完全透明的像素
			}

			// 调用绘制函数，直接传递RGB颜色值
			hdc_rectangle(x + i, y + j, a, a, RGB(r, g, b));
		}
	}
