...
{
    // 由于Teacher声明了friend Student，我们可以直接访问其私有成员
    num = 2150000 + (t.num % 1000);

    if (strlen(t.name) >= 4)
    {
        strcpy(name, "学生");
        strcat(name, t.name + 4);
    }
    else
    {
        strcpy(name, "学生");
...
{
    // 使用Student的数据构造新的Teacher对象
    int newNum = 21000 + (num % 1000);
    ...
    // 使用Teacher的构造函数创建对象
    return Teacher(newNum, newName, sex, newAddr);
}
