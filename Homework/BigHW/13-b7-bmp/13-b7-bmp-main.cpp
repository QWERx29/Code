...
int main(int argc, char **argv)
{
	...
	args_analyse_tools args[] = {
		...
		args_analyse_tools("--debug",     ST_EXTARGS_TYPE::boolean, 0, false), //用于查看调试信息，不需要实现（或者按自己的方式实现，对debug的输出不做要求）
	...
	bool is_debug = args[ARGS_DEBUG].existed();

	/* 通过带一参的构造函数方式，读取指定bmp的格式及所有点的颜色信息 */
	bitmap bmp(filename.c_str(), is_debug); //为了不让15-b8-bmp.h中包含头文件，特地改成char*形式
