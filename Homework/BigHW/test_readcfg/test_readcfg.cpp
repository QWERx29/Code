...
/***************************************************************************
  ...
  说    明：
 ***************************************************************************/
static void usage(const char* const argv0, const void* const ext_ptr, ostringstream& msg)
{
	/* 在路径字符串（类似"D:\\VS-Debug\\test-readcfg.exe")中查找最后一个\\，如果不存在，则为全称 */
	const char* procname = strrchr(argv0, '\\');
	if (procname == NULL)
		procname = argv0;
	else
		procname++; //指向\\的下一个字符

	msg << "e.g.   " << procname << " --cfgfile test.conf : 读取test.conf的所有组及每组的所有项" << endl;
	msg << endl;
	msg << "       " << procname << " --cfgfile test.conf --group [g1_name]       : 读取test.conf的[g1_name]组的所有项" << endl;
	msg << "       " << procname << " --cfgfile test.conf --group [g1_name] --gcs : 读取test.conf的[g1_name]组的所有项(组名不分大小写)" << endl;
	msg << endl;
	msg << "       " << procname << " --cfgfile test.conf --group [g1_name] --item item1                  : 读取test.conf的[g1_name]组的item1项的原始内容" << endl;
	msg << "       " << procname << " --cfgfile test.conf --group [g1_name] --item item1 --brkchar space  : 读取test.conf的[g1_name]组的item1项的原始内容(分隔符为空格/tab)" << endl;
	msg << "       " << procname << " --cfgfile test.conf --group [g1_name] --item item1 --type int       : 读取test.conf的[g1_name]组的item1项的值(int型)" << endl;
	msg << "       " << procname << " --cfgfile test.conf --group \"\"        --item item1 --type int --ics : 读取test.conf(不含组名的简单配置文件)的item1项的值(int型,项名不分大小写)" << endl;

	/* 不直接用cout，方便后期管理，比如不想马上输出，就可以先放在msg中，等待后续处理 */
	cout << msg.str() << endl;
}
...
int main(int argc, char* argv[])
{
	...
	/* =====检查参数之间的合理性=====
	   依据本题的实际情况，具体检查逻辑如下：
		1、对help做特殊处理，只要带--help，打印完立即退出
		2、对debug做特殊处理，只要带--debug，则打印参数列表
		3、必须带--cfgfile
		4、必须先有--group，才能--item
		5、只有--group需要--gcs
		6、只有--item需要--ics、--brkchar、--type
		7、如果是--item all，则不能指定--type
	*/ 
	...
	if (args[OPT_ARGS_GROUP_NAME].existed() == 0 && args[OPT_ARGS_ITEM_NAME].existed() != 0) {
		usage(argv[0], nullptr, msg);
		cout << "使用" << args[OPT_ARGS_ITEM_NAME].get_name() << "前必须先指定"
			 << args[OPT_ARGS_GROUP_NAME].get_name() << "(\"\"表示组名为空)" << endl;
		return -1;
	}

	/* 5、只有--group需要--gcs */
	if (args[OPT_ARGS_GROUP_NAME].existed() == 0 && args[OPT_ARGS_GROUP_NAME_CASE_SENSITIVE].existed() != 0) {
		usage(argv[0], nullptr, msg);
		cout << "使用" << args[OPT_ARGS_GROUP_NAME_CASE_SENSITIVE].get_name() << "前必须先指定"
			 << args[OPT_ARGS_GROUP_NAME].get_name() << "(\"\"表示组名为空)" << endl;
		return -1;
	}

	/* 6、只有--item需要--ics、--brkchar、--type*/
	if (args[OPT_ARGS_ITEM_NAME].existed() == 0 && 
			(args[OPT_ARGS_ITEM_NAME_CASE_SENSITIVE].existed() != 0 || 
			 args[OPT_ARGS_ITEM_BREAK_CHARACTER].existed() != 0 ||
			 args[OPT_ARGS_ITEM_TYPE].existed() != 0)	) {
		usage(argv[0], nullptr, msg);
		cout << "使用" << args[OPT_ARGS_ITEM_NAME_CASE_SENSITIVE].get_name() << "/"
			          << args[OPT_ARGS_ITEM_BREAK_CHARACTER].get_name() << "/"
			          << args[OPT_ARGS_ITEM_TYPE].get_name() << "前必须先指定" << args[OPT_ARGS_ITEM_NAME].get_name() << endl;
		return -1;
	}

	/* 7、如果是--item all，则不能指定--type */
	if (args[OPT_ARGS_ITEM_NAME].existed() && args[OPT_ARGS_ITEM_NAME].get_string() == "all" && args[OPT_ARGS_ITEM_TYPE].existed()) {
		usage(argv[0], nullptr, msg);
		cout << args[OPT_ARGS_ITEM_NAME].get_name() << " " << args[OPT_ARGS_ITEM_NAME].get_string()
			 << "不能跟" << args[OPT_ARGS_ITEM_TYPE].get_name() << endl;
		return -1;
	}

	/* =====参数之间的合理性检查完成=====
		注：因为每个具体程序的参数逻辑不同，每个程序必须写完全不同的检查语句，这个一个巨大的麻烦!!!
		    思考一下，有没有通用的方法
	*/
	...
	if (args[OPT_ARGS_GROUP_NAME].get_string() == "all") {
		...
		for (const auto& one_group : groups) {
			/* 取每组的所有项(同名组的所有项合并在一起) */
			vector <string> items;
			int items_num = sCfg.get_all_item(one_group, items, is_group_case_sensitive);

			cout << "组名 : " << (one_group.empty() ? "<NULL>" : one_group) << " 项数 : " << items_num << "/" << items.size() << endl; //简单配置文件的空组名打印为<NULL>，打印num和size()是为了核对项数是否正确
		...
		/* 如果 group 不是all且没有[]，则自动加一个（测试时为了方便而加，不是工具集函数中不需要支持）*/
		if (group_name_wanted != "" &&group_name_wanted != "all" && !(group_name_wanted[0] == '[' && group_name_wanted[group_name_wanted.length()-1] == ']'))
			group_name_wanted = "[" + group_name_wanted + "]";
		...
		if (args[OPT_ARGS_ITEM_NAME].get_string() == "all") {
			...
			if (items_num > 0) { //找到
				...
				cout << "未找到组" << group_name_wanted << "或组" << group_name_wanted << "的项数为0" << endl;
		}
			...
			if (item_type == "raw") {
				...
				if (item_type == "null") {
					...
					/* 先测试最简使用：无最大长度（自动为MAX_STRLEN，但实际数组只有16字节），、无缺省值、组名大小写不敏感、项名大小写不敏感
					...
					/* 其次测试有正确的最大长度、无缺省值的情况（因为需要--gcs和--ics，因此默认值只能设上）
