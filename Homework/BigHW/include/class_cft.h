...
/* 允许自定义其它需要的内容 */
//string st_string_get_procname(const char* const argv0);
...
private:
	...
	void init(const char* const _cfgname, const enum BREAK_CTYPE _ctype);
	bool read_success; //标记读配置文件是否成功
	bool is_empty;    //标记配置文件是否为空（全部是注释或空行）
	struct CFT_Item 
	{
		string raw;   // 项的有效内容（构造函数中解析出来）
	};
	struct CFT_Group 
	{
		string name;             // 组名，简单配置文件为 ""
		vector<CFT_Item> items;  // 此组下所有项（已按你的规则解析）
	};
	vector<CFT_Group> groups_;
	BREAK_CTYPE break_type_;
