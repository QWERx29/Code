...
private:
	...
	static void init(args_analyse_tools* obj, const char* name, const ST_EXTARGS_TYPE type, const int ext_num); //通用初始化函数
	static string getValue(const args_analyse_tools* arg, bool is_default);
	static string getTypeName(ST_EXTARGS_TYPE type);
	static string getRangeSet(const args_analyse_tools* arg, const int pos);
