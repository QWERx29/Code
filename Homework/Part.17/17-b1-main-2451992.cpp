//2451992 祝新豪 大数据
//Peers for cross-check: AAA BBB CCC
...
int main()
{
	...
	if (0) { // removed per 17-b1 requirement
		TStringAdv s1("tong"), s2 = "ji", s3;
	...
	if (1) {
		TStringAdv s1("abcdef"), s2, s3;

		cout << "TStringAdv 新界面测试：分配/追加/插入/删除/子字符串/获取/字符串长度" << endl;

		// assign()
		s2.assign(s1);
		cout << "s2 应该为 abcdef,    实际: " << s2 << endl;

		s2.assign("XYZ");
		cout << "s2 应该为 XYZ,       实际: " << s2 << endl;

		s2.assign("");
		cout << "s2 应该为 <EMPTY>,   实际: " << s2 << endl;

		s2.assign((const char*)nullptr);
		cout << "s2 应该为 <EMPTY>,   实际: " << s2 << endl;

		// append()
		s1 = "abc";
		s1.append("def").append('!');   // abcdef!
		cout << "s1 应该为 abcdef!,   实际: " << s1 << endl;

		s1.append((const char*)nullptr); // no change
		cout << "s1 应该为 abcdef!,   实际: " << s1 << endl;

		s3 = "++";
		s1.append(s3);
		cout << "s1 应该为 abcdef!++, 实际: " << s1 << endl << endl;

		// insert() : pos is 1-based, valid in [1..len+1]
		s1 = "abcd";
		s1.insert("XYZ", 1);
		cout << "insert at pos=1,       应该为 XYZabcd,          实际: " << s1 << endl;

		s1.insert("123", s1.length() + 1);
		cout << "insert at tail,        应该为 XYZabcd123,       实际: " << s1 << endl;

		s1.insert("", 3); // empty string: no insert
		cout << "insert empty string,   应该为 XYZabcd123,       实际: " << s1 << endl;

		s1.insert((const char*)nullptr, 3); // nullptr: no insert
		cout << "insert nullptr,        应该为 XYZabcd123,       实际: " << s1 << endl;

		s1.insert("BAD", 0); // invalid pos: no insert
		cout << "insert invalid pos=0,  应该为 XYZabcd123,       实际: " << s1 << endl;

		// insert(char) with '\0' : cut after pos-1; if pos==1 => empty
		s1 = "abcdef";
		s1.insert('\0', 4); // keep "abc"
		cout << "insert '\\0' at pos=4, 应该为 abc,              实际: " << s1 << endl;

		s1 = "abcdef";
		s1.insert('\0', 1); // empty
		cout << "insert '\\0' at pos=1, 应该为 <EMPTY>,          实际: " << s1 << endl;

		// erase() : remove first matched sub-string/char, equivalent to -=
		s1 = "This is a pencil";
		s1.erase("is");
		cout << "erase \"is\",          应该为 Th is a pencil,   实际: " << s1 << endl;

		s1.erase("is"); // erase next
		cout << "erase \"is\" again,    应该为 Th  a pencil,     实际: " << s1 << endl;

		s1.erase("not-exist"); // no change
		cout << "erase not-exist,       应该为 Th  a pencil,     实际: " << s1 << endl;

		s1 = "This is a pencil";
		s1.erase('i');
		cout << "erase 'i',             应该为 Ths is a pencil,  实际: " << s1 << endl << endl;

		// substr() : pos is 1-based, invalid -> empty; len<=0 -> empty; omit len -> to end
		s1 = "abcdef";
		cout << "substr(2,3)   应该为 bcd,     实际: " << s1.substr(2, 3) << endl;
		cout << "substr(3)     应该为 cdef,    实际: " << s1.substr(3) << endl;
		cout << "substr(0,2)   应该为 <EMPTY>, 实际: " << s1.substr(0, 2) << endl;
		cout << "substr(2,0)   应该为 <EMPTY>, 实际: " << s1.substr(2, 0) << endl;
		cout << "substr(5,100) 应该为 ef,      实际: " << s1.substr(5, 100) << endl << endl;

		// at() : 0-based, equivalent to operator[]
		s1 = "house";
		s1.at(0) = 'H';
		s1.at(1) = 'O';
		cout << "after at() modify,     应该为 HOuse, 实际: " << s1 << endl;

		// TStringAdvLen()
		s1 = "tong";
		cout << "TStringAdvLen(tong)    应该为 4,       实际: " << TStringAdvLen(s1) << endl;
		s2 = (const char*)nullptr;
		cout << "TStringAdvLen(<EMPTY>) 应该为 0,       实际: " << TStringAdvLen(s2) << endl;
		cout << "TStringAdvLen(tong+ji) 应该为 6,       实际: " << TStringAdvLen(s1 + "ji") << endl << endl;

		wait_for_enter();
	}
	if (0) { // optional 100MB tests removed for debugging
		memory_speed_test(0);
		memory_speed_test(1);
		memory_speed_test(2);
	}
