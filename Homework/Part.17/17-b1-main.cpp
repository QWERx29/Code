// 2451317 冯久恒 大数据
/* 2452214 霍宇哲 2452533 金一恺 2451992 祝新豪 */
...
int main()
{
    ...
    if (1)
    {
        ...
        s1.assign("NewString");
        cout << "s1应为NewString，实际输出：" << s1 << endl;

        s1.assign(s2);
        cout << "s1应为World，实际输出：" << s1 << endl;
    ...
    if (1)
    {
        ...
        cout << "TStringAdv append函数测试" << endl;
    ...
    if (1)
    {
        ...
        // 在开头插入
        s1.insert("Say: ", 1);
        cout << "s1应为Say: Hello World，实际输出：" << s1 << endl;
        ...
        // 插入空字符串
        TStringAdv emptyStr("");
        s1.insert(emptyStr, 3);
        cout << "s1仍为Say: Hello World!，实际输出：" << s1 << endl;

        // 插入字符
        s1.insert('X', 5);
        cout << "s1应为Say:X Hello World!，实际输出：" << s1 << endl;

        // 插入尾零（截断）
        s1.insert('\0', 6);
        cout << "s1应为Say:X，实际输出：" << s1 << endl;

        // 非法位置测试
        s1.insert("Test", 0);
        cout << "s1仍为Say:X，实际输出：" << s1 << endl;
    ...
    if (1)
    {
        TStringAdv s1("This is a test string for erase");

        cout << "TStringAdv erase函数测试" << endl;

        // 删除子串（只删除第一个匹配项）
        s1.erase("test ");
        cout << "s1应为This is a string for erase，实际输出：" << s1 << endl;

        // 删除第一个空格字符（不是所有空格）
        s1.erase(' ');
        cout << "s1应为Thisis a string for erase，实际输出：" << s1 << endl;

        // 删除不存在的子串，字符串不变
        s1.erase("notfound");
        cout << "s1不变，仍为Thisis a string for erase，实际输出：" << s1 << endl;
    ...
    if (1)
    {
        ...
        // 正常截取
        TStringAdv sub1 = s1.substr(1, 5);
        cout << "substr(1,5)应为Hello，实际输出：" << sub1 << endl;

        // 使用默认长度（到结尾）
        TStringAdv sub2 = s1.substr(7);
        cout << "substr(7)应为World!，实际输出：" << sub2 << endl;

        // 长度超出
        TStringAdv sub3 = s1.substr(7, 20);
        cout << "substr(7,20)应为World!，实际输出：" << sub3 << endl;

        // 非法位置
        TStringAdv sub4 = s1.substr(0, 5);
        cout << "substr(0,5)应为<EMPTY>，实际输出：" << sub4 << endl;

        TStringAdv sub5 = s1.substr(20, 5);
        cout << "substr(20,5)应为<EMPTY>，实际输出：" << sub5 << endl;

        // 长度为0或负数
        TStringAdv sub6 = s1.substr(1, 0);
        cout << "substr(1,0)应为<EMPTY>，实际输出：" << sub6 << endl;

        TStringAdv sub7 = s1.substr(1, -5);
        cout << "substr(1,-5)应为<EMPTY>，实际输出：" << sub7 << endl;
    ...
    if (1)
    {
        ...
        TStringAdv s2("House");
        s2.at(1) = 'O';
        cout << "after at() modify, 应该为 HOuse, 实际: " << s2 << endl;
    ...
    if (1)
    {
        TStringAdv s1("Test String");

        cout << "TStringAdvLen函数测试" << endl;

        cout << "s1长度为" << TStringAdvLen(s1) << "，应与" << s1.length() << "相同" << endl;

        TStringAdv empty;
        cout << "空字符串长度为" << TStringAdvLen(empty) << endl;
