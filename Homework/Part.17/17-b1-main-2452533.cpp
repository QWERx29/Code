...
int main()
{
	...
	// 测试assign函数
	if (1) {
		cout << "测试assign函数:" << endl;
		TStringAdv s1("house");

		// 测试读取
		cout << "s1 = \"house\"" << endl;
		cout << "s1.at(0) 应为 'h', 实际: " << s1.at(0) << endl;
		cout << "s1.at(1) 应为 'o', 实际: " << s1.at(1) << endl;
		cout << "s1.at(4) 应为 'e', 实际: " << s1.at(4) << endl;

		// 测试修改
		s1.at(0) = 'H';
		cout << "s1.at(0) = 'H' 后，s1应为 House, 实际: " << s1 << endl;

		s1.at(1) = 'O';
		cout << "s1.at(1) = 'O' 后，s1应为 HOuse, 实际: " << s1 << endl;

		// 测试越界
		try {
			char c = s1.at(10); // 越界访问
			cout << "s1.at(10) 不应执行到此，实际得到: " << c << endl;
		}
		catch (const std::exception& e) {
			cout << "s1.at(10) 正确抛出异常: " << e.what() << endl;
		}
	}
	...
	if (1) {
		...
		s1.append(" World");
		cout << "s1.append(\" World\")后，s1应为Hello World，实际输出：" << s1 << endl;

		TStringAdv s2("!");
		s1.append(s2);
		cout << "s1.append(s2)后，s1应为Hello World!，实际输出：" << s1 << endl;

		s1.append('?');
		cout << "s1.append('?')后，s1应为Hello World!?，实际输出：" << s1 << endl;
	...
	if (1) {
		...
		s1.insert(" ", 6);
		cout << "s1.insert(\" \", 6)后，s1应为Hello World，实际输出：" << s1 << endl;

		TStringAdv s2("Beautiful ");
		s1.insert(s2, 7);
		cout << "s1.insert(s2, 7)后，s1应为Hello Beautiful World，实际输出：" << s1 << endl;

		s1.insert('!', 100);  // 测试越界
		cout << "s1.insert('!', 100)越界测试，s1应为Hello Beautiful World，实际输出：" << s1 << endl;

		s1.insert('\0', 1);  // 测试pos=1插入'\0'
		cout << "s1.insert('\\0', 1)后，s1应为<EMPTY>，实际输出：" << s1 << endl;
	...
	if (1) {
		cout << "测试erase函数:" << endl;
		TStringAdv s1("Hello Beautiful World!");

		s1.erase("Beautiful ");
		cout << "s1.erase(\"Beautiful \")后，s1应为Hello World!，实际输出：" << s1 << endl;

		TStringAdv s2("World");
		s1.erase(s2);
		cout << "s1.erase(s2)后，s1应为Hello !，实际输出：" << s1 << endl;

		s1.erase('!');
		cout << "s1.erase('!')后，s1应为Hello ，实际输出：" << s1 << endl;

		// 测试不存在的删除
		s1.erase("NotExist");
		cout << "s1.erase(\"NotExist\")后，s1应为Hello，实际输出：" << s1 << endl;
	...
	// 测试substr函数
	if (1) {
		cout << "测试substr函数:" << endl;
		TStringAdv s1("Hello World!");

		TStringAdv s2 = s1.substr(7);
		cout << "s1.substr(7)后，s2应为World!，实际输出：" << s2 << endl;

		TStringAdv s3 = s1.substr(7, 5);
		cout << "s1.substr(7, 5)后，s3应为World，实际输出：" << s3 << endl;

		// 测试越界
		TStringAdv s4 = s1.substr(100);
		cout << "s1.substr(100)越界测试，s4应为<EMPTY>，实际输出：" << s4 << endl;

		TStringAdv s5 = s1.substr(7, 100);
		cout << "s1.substr(7, 100)超长测试，s5应为World!，实际输出：" << s5 << endl;
	...
	if (1) {
		...
		cout << "s1.at(0)应为H，实际输出：" << s1.at(0) << endl;
		cout << "s1.at(4)应为o，实际输出：" << s1.at(4) << endl;

		s1.at(0) = 'h';
		cout << "s1.at(0)='h'后，s1应为hello，实际输出：" << s1 << endl;
